...
@
\\\textit{The first two principal components explain the 21\% and the 15\% of the variability respectively.}
...
@
\\\textit{This pattern explains the 96\% of the variability. Sample GSM487042 is the most different one.}
...
# Centering the values under the null hypothesis
diff <- t(scale(t(diff)))
# Bootstrap
boot <- pbapply::pbsapply(1:100, function(b){
    apply(diff[,sample(1:5,replace = TRUE)],1,function(X){t.test(X)$statistic})
})
# p-Value
pValues <- rowMeans(abs(boot) > abs(tStat))
